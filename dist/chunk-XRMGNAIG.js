...
function normalizeNetwork(network) {
  ...
  const name = typeof network === "number" ? { 1: "mainnet", 2: "testnet", 3: "devnet", 4: "local" }[network] ?? void 0 : network;
  if (!name) {
    throw new NovaAdapterError("INVALID_NETWORK" /* InvalidNetwork */, `Unsupported network value: ${String(network)}`);
  }
  const chainId = typeof network === "number" ? network : { mainnet: 1, testnet: 2, devnet: 3, local: 4 }[name] ?? void 0;
  return {
    name,
    chainId
  };
...
var NovaClient = class extends EventEmitter {
  ...
  async signMessageAndVerify(input) {
    ...
    const network = await this.network().catch(() => null);
    const output = await this.signMessage(input);
    const message = new TextEncoder().encode(
      output.fullMessage || createFullMessage(input, account.address.toString(), network?.chainId)
    );
    if ("verifySignatureAsync" in account.publicKey) {
      return account.publicKey.verifySignatureAsync({
        message,
        signature: output.signature
      });
    }
    return account.publicKey.verifySignature({
      message,
      signature: output.signature
    });
  }
  ...
  async signAndSubmitTransaction(transaction, options) {
    try {
      ...
      const submitted = await submitSignedTransaction({
        network: await this.network().catch(() => null),
    ...
    "cedra:signTransaction": {
      ...
      signTransaction: async (input) => {
        const result = await client.signTransaction({
          ...
          options: {
            expirationSecondsFromNow: input.expirationSecondsFromNow,
            expirationTimestamp: input.expirationTimestamp,
            gasUnitPrice: input.gasUnitPrice,
            maxGasAmount: input.maxGasAmount,
            accountSequenceNumber: typeof input.sequenceNumber === "bigint" ? Number(input.sequenceNumber) : input.sequenceNumber
          }
        });
        if (!result || typeof result !== "object" || !("authenticator" in result)) {
          throw new Error("Nova signTransaction did not return an authenticator");
        }
        return {
          status: UserResponseStatus.APPROVED,
          args: {
            ...
            rawTransaction: {
              bcsToBytes: () => result.rawTransaction ?? new Uint8Array()
            }
          }
        };
      }
